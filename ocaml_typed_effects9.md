...
Because `Effect.t` is an extensible GADT indexed by resumption type (`'res Effect.t`)[cite: 4], each pattern branch in `match e with effect ...` introduces an existential type index[cite: 4, 5].

In `typecore.ml` (`type_effect_cases`), **each effect case must be typed in an independent existential scope**:
1. For each branch $i$ matching `effect pat, k`:
   * Allocate a **fresh, independent existential variable** $\alpha_{\text{res}}^{(i)}$[cite: 4].
   * Type the pattern `pat` against `\alpha_{\text{res}}^{(i)} Effect.t`.
   * Bind continuation $k$ in that branch's local environment with argument type $\alpha_{\text{res}}^{(i)}$[cite: 4]:
     $$k : (\alpha_{\text{res}}^{(i)}, \tau_1) \text{ continuation}$$
2. **No Cross-Case Leakage:** Resumption variables $\alpha_{\text{res}}^{(i)}$ and $\alpha_{\text{res}}^{(j)}$ ($i \neq j$) must **never be unified together**.
3. **Escape Check:** Verify that $\alpha_{\text{res}}^{(i)}$ does not escape into the branch body's return type $\tau_{\text{out}}$ or the outer environment.
...
* In `typecore.ml`:
  ...
  * In `type_effect_cases`:
    * Instantiate a **per-branch** existential type variable $\alpha_{\text{res}}$ for each `effect pat, k` arm rather than sharing a single type variable across the entire `match` construct[cite: 4].
    * Open a local definition scope per arm (`Ctype.begin_def()`), unpack the GADT constructor equation into the local branch environment, bind $k$ with that local resumption type[cite: 4], and verify non-escape upon closing the branch scope.
