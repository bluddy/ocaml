...
### 3.4 Elimination Rule (Typing Effect Handlers)
...
The typing judgment enforces row subtraction on the handled body:
...
* The handled label $\ell$ is removed from the outer effect row ($R = \text{Effects}(e) \setminus \{ \ell \}$).
* The continuation $k$ carries the remaining tail $R$ on its arrow.
* The outer expression inherits only remaining unhandled effects $R$ plus any new effects introduced by the handler bodies ($R_{\text{ret}}, R_{\text{eff}}$).
...
* $\ell_{\text{in}}$ is discharged by the internal handler.
...
* In `type_match`, detect `effect` clauses, perform row subtraction ($R \setminus \ell$), and type continuation $k$ with the remaining tail $R$.
...
* Stack-allocate local handler state contexts using `@local` modes to eliminate GC heap churn.
